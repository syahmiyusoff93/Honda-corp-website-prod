...
if(!class_exists('MNC7000')){
    class MNC7000 {
        public function itemList($info) {
            ...
            $disp[1] .= '<div class="ttl">'.$info['title'][0].'</div>
                        <div class="desc">'.$info['title'][1].'</div>';
            $btnClose = '<a class="btn-gen ccl">Close</a>';
            $disp[1]=empty(strip_tags($info['content']))?$disp[1].$btnClose:'<div class="col-lg-7">' .$disp[1].'<div class="cont wrap">'.$info['content'].'</div>'.$btnClose. '</div>';
            
            $info['bgurl']=empty($info['content'])?'<div class="col-lg-12"><p style="text-align:center;"><img src="'.$info['bgurl'].'" alt=""></p></div>':'<div class="col-lg-5"><p><img src="'.$info['bgurl'].'" alt=""></p></div>';
            
            $temp = '<div class="row">'.$info['bgurl'].$disp[1].'</div>';
            
            $disp[0] = '<div class="f">'.$disp[0].'<template>'.$temp.'</template></div>';
            
            return '<div class="itm">'.$disp[0].'</div>';
        }
    }
}
$MNC7000 = new MNC7000();