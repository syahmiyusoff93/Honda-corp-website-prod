<?php
if(!class_exists('MNC3001')){
    class MNC3001 extends INFOEXT{
        public function groupList($CON, $item) {
            $link = '';
            if( $item['link']['redir'] ) {
                $link = $this->button($link, 'btn-gen'); 
            } 
            return '<div class="infobx-itm">
                <div class="wrap f">
                    <div class="itm-ttl">'.$item['title'].'</div>
                    <div class="taste">'.$link.'</div>
                </div>
            </div>';
        }
